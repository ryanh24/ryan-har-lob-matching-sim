...
void Book::remove_order(Order* o) {
    ...
    if (level->head == nullptr) { // level emptied → remove that Limit from its side
        // Side detection: a price lives on only one side of an uncrossed book.
        PriceTree& tree = (bids_.find(level->price) != nullptr) ? bids_ : asks_;
        tree.erase(level->price); // frees the Limit node (level->price read before erase runs)
    }
}
